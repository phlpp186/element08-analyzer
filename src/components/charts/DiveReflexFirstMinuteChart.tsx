...
export function DiveReflexFirstMinuteChart({ points }: Props) {
  ...
  if (points.length === 0) {
    return (
      <ChartCard
        title={t('Dive Reflex: First Minute')}
        description={t('Average HR over the first minute of every hold.')}
      >
        <p className="py-8 text-center text-sm text-textDim">
          {t('No holds with HR data long enough (≥ 30s) in this backup yet.')}
        </p>
      </ChartCard>
    );
  }
  ...
  return (
    <ChartCard
      title={t('Dive Reflex: First Minute')}
      description={t('Average HR across all holds (≥ 30s), sampled every 5s. The downward slope is the bradycardic dive reflex.')}
    >
